...
class LogWrapper(JaxMARLWrapper):
    ...
    def step(
        ...
        new_episode_return = state.episode_returns + self._batchify_floats(
            reward)
...
class MPELogWrapper(LogWrapper):
    ...
    def step(
        ...
        obs, env_state, reward, done, info = self._env.step(
            key, state.env_state, action)
        rewardlog = jax.tree_map(lambda x: x * self._env.num_agents,
                                 reward)  # As per on-policy codebase
        ep_done = done["__all__"]
        new_episode_return = state.episode_returns + self._batchify_floats(
            rewardlog)
        new_episode_length = state.episode_lengths + 1
...
class CTRolloutManager(JaxMARLWrapper):
    ...
    def __init__(self,
        ...
        self.batch_samplers = {
            agent: jax.jit(jax.vmap(self.action_space(agent).sample,
                                    in_axes=0))
            for agent in self.agents
        }
        ...
        self.max_obs_length = max(
            list(
                map(lambda x: get_space_dim(x),
                    self.observation_spaces.values())))
        ...
        self.agents_one_hot = {
            a: oh
            for a, oh in zip(self.agents, jnp.eye(len(self.agents)))
        }
        # valid actions
        self.valid_actions = {
            a: jnp.arange(u.n)
        ...
        if 'smax' in env.name.lower():
            ...
            self.global_reward = lambda rewards: rewards[self.training_agents[
                0]]
    ...
    def batch_step(self, key, states, actions):
        keys = jax.random.split(key, self.batch_size)
        return jax.vmap(self.wrapped_step, in_axes=(0, 0, 0))(keys, states,
                                                              actions)
    ...
    def wrapped_reset(self, key):
        ...
        if self.preprocess_obs:
            obs = jax.tree_util.tree_map(
                ...
                {agent: obs_[agent]
                 for agent in self.agents}, self.agents_one_hot)
        else:
            obs = obs_
    ...
    def wrapped_step(self, key, state, actions):
        if 'hanabi' in self._env.name.lower():
            actions = jax.tree_util.tree_map(lambda x: jnp.expand_dims(x, 0),
                                             actions)
    ...
    def global_reward(self, reward):
        return jnp.stack([reward[agent]
                          for agent in self.training_agents]).sum(axis=0)

    def batch_sample(self, key, agent):
        return self.batch_samplers[agent](jax.random.split(
            key, self.batch_size)).astype(int)
    ...
    def hanabi_world_state(self, obs, state):
        ...
        hands = state.env_state.player_hands.reshape(
