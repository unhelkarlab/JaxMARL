...
OBJECT_INDEX_TO_VEC = jnp.array([
    ...
    jnp.array([OBJECT_TO_INDEX['wall'], COLOR_TO_INDEX['grey'], 0],
              dtype=jnp.uint8),
    jnp.array([OBJECT_TO_INDEX['onion'], COLOR_TO_INDEX["yellow"], 0],
              dtype=jnp.uint8),
    jnp.array([OBJECT_TO_INDEX['onion_pile'], COLOR_TO_INDEX["yellow"], 0],
              dtype=jnp.uint8),
    jnp.array([OBJECT_TO_INDEX['plate'], COLOR_TO_INDEX["white"], 0],
              dtype=jnp.uint8),
    jnp.array([OBJECT_TO_INDEX['plate_pile'], COLOR_TO_INDEX["white"], 0],
              dtype=jnp.uint8),
...
def make_overcooked_map(wall_map,
                        goal_pos,
                        agent_pos,
                        agent_dir_idx,
                        plate_pile_pos,
    ...
    wall = jnp.array([OBJECT_TO_INDEX['wall'], COLOR_TO_INDEX['grey'], 0],
                     dtype=jnp.uint8)
    # Expand 'wall_map' (dimension is H x W) to H x W x 1
    maze_map = jnp.array(jnp.expand_dims(wall_map, -1), dtype=jnp.uint8)
    # Expand 'maze_map' to H x W x 3
    # The last dimension contains object index, color index, and state index
    maze_map = jnp.where(maze_map > 0, wall, empty)

    # Add agents
    def _get_agent_updates(agent_dir_idx, agent_pos, agent_idx):
        agent = jnp.array([
            OBJECT_TO_INDEX['agent'], COLOR_TO_INDEX['red'] + agent_idx * 2,
            agent_dir_idx
        ],
                          dtype=jnp.uint8)
    ...
    # 'agent_x_vec' contains the 'x' coord of the two agents
    # 'agent_vec' is a 2D array. Each row in the array corresponds to an
    # agent's info
    agent_x_vec, agent_y_vec, agent_vec = jax.vmap(
        _get_agent_updates, in_axes=(0, 0, 0))(agent_dir_idx, agent_pos,
                                               jnp.arange(num_agents))
    maze_map = maze_map.at[agent_y_vec, agent_x_vec, :].set(agent_vec)

    # Add goals
    goal = jnp.array([OBJECT_TO_INDEX['goal'], COLOR_TO_INDEX['green'], 0],
                     dtype=jnp.uint8)
    goal_x = goal_pos[:, 0]
    goal_y = goal_pos[:, 1]
    ...
    onion_pile = jnp.array(
        [OBJECT_TO_INDEX['onion_pile'], COLOR_TO_INDEX["yellow"], 0],
        dtype=jnp.uint8)
    ...
    plate_pile = jnp.array(
        [OBJECT_TO_INDEX['plate_pile'], COLOR_TO_INDEX["white"], 0],
        dtype=jnp.uint8)
    ...
    pot_y = pot_pos[:, 1]
    pots = jnp.stack([
        jnp.array([OBJECT_TO_INDEX['pot'], COLOR_TO_INDEX["black"], status])
        for status in pot_status
    ],
                     dtype=jnp.uint8)
    ...
    if len(onion_pos) > 0:
        ...
        onion_y = onion_pos[:, 1]
        onion = jnp.array(
            [OBJECT_TO_INDEX['onion'], COLOR_TO_INDEX["yellow"], 0],
            dtype=jnp.uint8)
    ...
    if len(plate_pos) > 0:
        ...
        plate_y = plate_pos[:, 1]
        plate = jnp.array(
            [OBJECT_TO_INDEX['plate'], COLOR_TO_INDEX["white"], 0],
            dtype=jnp.uint8)
        maze_map = maze_map.at[plate_y, plate_x, :].set(plate)
    if len(dish_pos) > 0:
        dish_x = dish_pos[:, 0]
        dish_y = dish_pos[:, 1]
        dish = jnp.array([OBJECT_TO_INDEX['dish'], COLOR_TO_INDEX["white"], 0],
                         dtype=jnp.uint8)
    ...
    # jnp.tile() constructs an array by repeating the first parameter a number
    # of times given by the second parameter
    # The second argument is the number of repititions along each axis
    maze_map_padded = jnp.tile(
        wall.reshape((1, 1, *empty.shape)),
        (maze_map.shape[0] + 2 * padding, maze_map.shape[1] + 2 * padding, 1))
    maze_map_padded = maze_map_padded.at[padding:-padding,
                                         padding:-padding, :].set(maze_map)
    ...
    maze_map_padded = maze_map_padded.at[wall_start,
                                         wall_start:wall_end_x + 1, :].set(
                                             wall)  # top
    maze_map_padded = maze_map_padded.at[wall_end_y,
                                         wall_start:wall_end_x + 1, :].set(
                                             wall)  # bottom
    maze_map_padded = maze_map_padded.at[wall_start:wall_end_y + 1,
                                         wall_start, :].set(wall)  # left
    maze_map_padded = maze_map_padded.at[wall_start:wall_end_y + 1,
                                         wall_end_x, :].set(wall)  # right
