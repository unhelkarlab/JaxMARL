...
class Discrete(Space):
    ...
    def sample(self, rng: chex.PRNGKey) -> chex.Array:
        ...
        return jax.random.randint(rng,
                                  shape=self.shape,
                                  minval=0,
                                  maxval=self.n).astype(self.dtype)
...
class Box(Space):
    ...
    def sample(self, rng: chex.PRNGKey) -> chex.Array:
        ...
        return jax.random.uniform(rng,
                                  shape=self.shape,
                                  minval=self.low,
                                  maxval=self.high).astype(self.dtype)

    def contains(self, x: jnp.int_) -> bool:
        """Check whether specific object is within space."""
        # type_cond = isinstance(x, self.dtype)
        # shape_cond = (x.shape == self.shape)
        range_cond = jnp.logical_and(jnp.all(x >= self.low),
                                     jnp.all(x <= self.high))
        return range_cond
...
class Dict(Space):
    ...
    def sample(self, rng: chex.PRNGKey) -> dict:
        ...
        return OrderedDict([(k, self.spaces[k].sample(key_split[i]))
